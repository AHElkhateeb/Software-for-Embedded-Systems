...
/** 
 * Time struct to convert the time value to more human read form
 */
struct time_t {
uint8_t hour;
uint8_t minute;
...
/**
 * Return the time value that is stored in SystemTime_t variable
 * */
systemTime_t scheduler_getTime();

/**
 * Allow writing new time to the variable used for tracking the time
 * */
